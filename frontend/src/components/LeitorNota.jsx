...
export default function LeitorNota() {
  ...
  const handleLeitura = async (e) => {
    if (e.key === 'Enter') {
      // Limpa os estados visuais antes de processar nova leitura
      setMensagem('');
      setMensagemFluxo('');
      setStatus('');
      setNota(null);
      ...
      try {
        const { data } = await axios.post('http://localhost:3001/api/gravar-nota', {
          chave_acesso: chave,
  ...
  const finalizarNota = async () => {
    if (!nota?.id) return;
    try {
      await axios.put(`http://localhost:3001/api/finalizar-nota/${nota.id}`);
  ...
  const finalizarContainer = async () => {
    if (!nota?.id) return;
    try {
      await axios.put(`http://localhost:3001/api/finalizar-container/${nota.id}`);
